...
fn set_agents_tracks_only_the_pinned_agent() {
    ...
    a.set_agents(vec![
        agent("w1:p2", "idle", "/repo/web"),
        agent("w1:p1", "blocked", "/repo/api"),
    ]);
    assert_eq!(a.agent().unwrap().pane_id, "w1:p1");
...
fn set_diff_preserves_file_selection_by_path() {
    ...
    assert_eq!(
        a.files_tree_rows()[a.selected_file].path,
        "user.py",
...
fn diff_shows_whole_file_when_content_is_set() {
    ...
    assert!(
        matches!(&rows[41], Row::Line(l) if l.kind == LineKind::Del && l.text == "    return None")
    );
    assert!(matches!(&rows[42], Row::Line(l) if l.kind == LineKind::Add && l.new_no == Some(42)));
    assert!(
        matches!(&rows[44], Row::Line(l) if l.kind == LineKind::Context && l.new_no == Some(44))
    );
}
...
fn agent_header_is_not_interactive() {
    ...
    assert_eq!(
        a.handle_mouse(Mouse::LeftClick, 3, 2, &regions()),
        Action::None
    );
    assert_eq!(
        a.handle_mouse(Mouse::WheelDown, 3, 2, &regions()),
        Action::None
    );
    assert_eq!(
        a.agent().unwrap().pane_id,
        before,
        "pinned agent never changes"
    );
...
fn viewer_comment_on_any_line_including_unchanged() {
    ...
    assert_eq!(a.handle_key('c'), Action::None);
    assert!(
        matches!(a.modal, Some(Modal::Input { .. })),
        "input opens in viewer"
    );
...
fn viewer_comment_uses_repo_path_in_multi_mode() {
    ...
    let comments = a.store.comments(&a.agent_key());
    assert_eq!(
        comments[0].path, "/repo/api/user.py",
...
fn changed_files_render_as_tree() {
    ...
    assert_eq!(
        names,
        vec![("src", true), ("app.rs", false), ("README.md", false)]
    );
}
...
fn changed_tree_dir_toggles_and_file_shows_diff() {
    ...
    assert!(
        matches!(&a.rows()[1], Row::Line(l) if l.text == "old"),
...
fn changed_tree_click_file_shows_its_diff() {
    ...
    assert_eq!(a.right, RightPane::Diff);
    assert!(
        matches!(&a.rows()[1], Row::Line(l) if l.text == "a"),
...
fn single_repo_group_keeps_plain_tree() {
    ...
    let rows = a.files_tree_rows();
    assert_eq!(
        rows[0].name, "config.py",
        "no repo prefix for a single repo"
    );
